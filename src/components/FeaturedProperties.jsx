...
import { MapPin, Star, Wifi, Loader2, Zap } from 'lucide-react';
            ...
            {properties.map((property) => (
              <motion.div key={property.id} onClick={() => navigate(`/property/${property.id}`)} className="bg-white rounded-[40px] overflow-hidden shadow-sm hover:shadow-2xl transition-all border border-gray-100 flex flex-col h-full group">
                <div className="relative h-64 overflow-hidden">
                  <img src={property.photo_url || "/placeholder.jpg"} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 hover:scale-105" />
                ...
                <div className="p-8 flex flex-col flex-grow">
                  <h3 className="font-black italic text-xl text-gray-900 mb-2 uppercase tracking-tight leading-none line-clamp-1">{property.name}</h3>
                  <div className="flex items-center gap-2 text-gray-400 mb-6">
                    <MapPin size={14} className="text-black shrink-0" />
                    <span className="text-[11px] font-bold uppercase italic tracking-wider line-clamp-1">{property.address}</span>
                  </div>
                  ...
                  <div className="mt-auto flex items-center justify-between pt-6 border-t border-gray-50">
                    ...
                    <Button onClick={(e) => { e.stopPropagation(); onBookNow(property); }} className="bg-black hover:bg-gray-800 text-white rounded-2xl h-11 px-6 text-[10px] font-black uppercase italic shadow-lg">Pesan</Button>
                  </div>
                </div>
              </motion.div>
            ))}
