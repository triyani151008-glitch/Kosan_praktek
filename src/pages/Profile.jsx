...
const Profile = () => {
  ...
  const handleRequestOTP = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setOtpSent(true);
      toast({ title: "OTP Dikirim", description: "Cek SMS verifikasi Anda." });
    } catch (error) { toast({ variant: "destructive", description: error.message }); }
    finally { setUpdating(false); }
  };
      ...
      <div className="container mx-auto px-4 max-w-2xl">
        ...
        <div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm min-h-[450px] mb-6">
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'keamanan' && (
              <motion.div key="security" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <Button onClick={handleUpdateName} disabled={updating} className="w-full bg-black text-white rounded-xl h-12 font-bold uppercase text-[11px] tracking-widest shadow-lg">Update Nama</Button>
                </div>

                {/* 2. SEKSI PONSEL DENGAN PEMILIH NEGARA */}
                <div className="space-y-4 pt-4 border-t border-gray-50 relative">
                  ...
                  <div className="flex gap-2 items-center">
                    <div className="relative">
                      <button onClick={() => setShowCountryPicker(!showCountryPicker)} className="flex items-center gap-2 px-3 h-12 rounded-xl border border-gray-100 bg-white shadow-sm min-w-[105px] justify-center hover:bg-gray-50 transition-all">
                        <img src={`https://flagcdn.com/w40/${selectedCountry.flag}.png`} alt="flag" className="w-5 h-3 object-cover rounded-sm shadow-sm" />
                        ...
                        <div className="absolute top-full left-0 w-[200px] bg-white border border-gray-100 rounded-2xl shadow-2xl z-50 p-2 mt-2 space-y-1">
                          {countries.map((c) => (
                            <button key={c.flag} onClick={() => { setSelectedCountry(c); setShowCountryPicker(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-colors">
                              ...
                              <span className="text-xs font-bold text-gray-500">{c.name} ({c.code})</span>
                            </button>
                          ))}
                        </div>
                      )}
                    </div>
                    <Input value={securityData.phone} onChange={(e) => setSecurityData({...securityData, phone: e.target.value})} className="h-12 rounded-xl border-gray-100 bg-white font-bold flex-1" placeholder="81..." />
                  </div>
            ...
            {/* TAB PLACEHOLDER LAINNYA */}
            {activeTab === 'profil' && (
              <motion.div key="profile" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                <h3 className="text-lg font-black uppercase border-b border-gray-100 pb-4">Profil & Identitas</h3>
                <div className="p-12 border-2 border-dashed border-gray-100 rounded-[32px] flex flex-col items-center justify-center bg-gray-50 hover:bg-white transition-colors cursor-pointer group">
                  ...
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic text-center">Unggah KTP Sesuai Identitas</span>
                </div>
